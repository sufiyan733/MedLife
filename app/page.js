...
function ConfirmBedModal({ hospital, onClose }) {
  ...
  return (
    ...
    >
      <div
        className="w-full sm:max-w-md bg-white overflow-hidden flex flex-col"
        style={{
      ...
      <div className="p-4 sm:p-5 flex flex-col gap-3 flex-1">
        <div className="flex items-start justify-between gap-2">
          <div className="flex items-center gap-2.5 min-w-0 flex-1">
            ...
            <div className="min-w-0">
              <h3 className="font-bold text-slate-900 text-[13px] leading-tight truncate" style={{ fontFamily: "'Sora',sans-serif" }}>
                {h.name}
              </h3>
        ...
        <div className="flex items-center flex-wrap gap-1.5">
          ...
          <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full" style={{ background: waitColor.bg, color: waitColor.text }}>
            ⏱ ~{h.waitTime}m
          </span>
          {h.emergency && (
            <>
          ...
          {h.distanceKm !== undefined && (
            <>
              <span className="w-px h-3.5 bg-slate-200" />
              <span className="text-[11px] font-semibold text-slate-400">📍 {h.distanceKm.toFixed(1)} km</span>
            </>
          )}
        </div>
        ...
        <div className="flex flex-wrap gap-1">
          {h.specialties.map((s) => (
            <span key={s} className="text-[10px] font-medium px-2 py-0.5 rounded-full" style={{ background: "#f8fafc", border: "1px solid #e2e8f0", color: "#475569" }}>
              {s}
            </span>
          ))}
        </div>
...
export default function LandingPage() {
  ...
  const filtered = filter==="All" ? validHospitals : validHospitals.filter((h) => { const keywords=FILTER_MAP[filter]||[filter.toLowerCase()]; return h.specialties.some((s) => keywords.some((kw) => s.toLowerCase().includes(kw))); });
    ...
    <div className="min-h-screen" style={{ background:"#f8faf9", fontFamily:"'DM Sans', sans-serif" }}>
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6">
          ...
          <div className="flex gap-4 lg:gap-5 items-start">
                  ...
                  <div className={`grid gap-3 sm:gap-4 ${gridCols}`}>
                    {filtered.map((h) => (
                      <div key={h.id} className={highlightedIds.includes(h.id) ? "highlighted" : ""}>
                        <HospitalCard h={h} onSelect={handleMapClick} isSelected={mapOpen && selectedHospital?.id===h.id} userLat={userLocation?.lat} userLng={userLocation?.lng} />
                      </div>
                    ))}
                  </div>
                )
            ...
            {mapAsSidePanel && selectedHospital && (
              <div className="w-1/2 sticky top-20 rounded-2xl overflow-hidden shadow-[0_8px_40px_rgba(0,0,0,0.1)] slide-right flex flex-col" style={{ height:"calc(100vh - 5.5rem)", border:"1.5px solid #edf0f2" }}>
                <MapPanel hospital={selectedHospital} onClose={() => { setMapOpen(false); setSelectedHospital(null); }} userLat={userLocation?.lat} userLng={userLocation?.lng} isMobileSheet={false} />
              </div>
            )}
          </div>

          {mapAsSheet && selectedHospital && (
            <MapPanel hospital={selectedHospital} onClose={() => { setMapOpen(false); setSelectedHospital(null); }} userLat={userLocation?.lat} userLng={userLocation?.lng} isMobileSheet={true} />
          )}
        </div>
      </section>
      ...
      <section id="emergency" className="py-12 sm:py-16" style={{ background:"linear-gradient(135deg,#dc2626,#b91c1c)" }}>
        <div className="max-w-3xl mx-auto px-4 sm:px-6 text-center">
          ...
          <button onClick={() => setEmergencyOpen(true)} className="pulse-ring inline-flex items-center gap-2 sm:gap-3 px-6 sm:px-8 py-3.5 sm:py-4 bg-white text-rose-600 font-extrabold text-[14px] sm:text-[15px] rounded-2xl hover:shadow-2xl transition-all hover:-translate-y-px active:scale-95 w-full sm:w-auto justify-center">
