      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 flex-shrink-0">
          <div className="min-w-0 flex-1 mr-3">
            ...
            <h3 className="font-bold text-slate-900 text-[15px] leading-tight truncate" style={{ fontFamily: "'Sora',sans-serif" }}>{hospital.name}</h3>
        ...
        <div className="p-4 sm:p-5 flex flex-col gap-3 flex-1">
          <div className="flex items-start justify-between gap-2">
            <div className="flex items-center gap-2.5 min-w-0 flex-1">
              <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center text-white font-black text-[11px] flex-shrink-0 shadow-sm" style={{ background: h.accentHex }}>
                {h.shortName}
              </div>
              <div className="min-w-0">
                <h3 className="font-bold text-slate-900 text-[13px] leading-tight truncate" style={{ fontFamily: "'Sora',sans-serif" }}>{h.name}</h3>
                <p className="text-[10px] text-slate-400 mt-0.5">{h.type}</p>
              </div>
            </div>
            <span className="text-[9px] font-black uppercase tracking-[0.1em] px-2 py-1 rounded-full flex-shrink-0"
          ...
          <div className="flex items-center flex-wrap gap-1.5">
            ...
            <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full" style={{ background: waitColor.bg, color: waitColor.text }}>⏱ ~{h.waitTime}m</span>
          ...
          <div className="flex flex-wrap gap-1">
            {h.specialties.map(s => (
              <span key={s} className="text-[10px] font-medium px-2 py-0.5 rounded-full" style={{ background: "#f8fafc", border: "1px solid #e2e8f0", color: "#475569" }}>{s}</span>
            ))}
          </div>

          <div className="flex items-center justify-between pt-1 border-t border-slate-50 gap-2">
            <p className="text-[10px] text-slate-400 truncate flex-1 min-w-0">{h.address}</p>
      ...
      <div className="p-4 sm:p-5" style={{ background: hospital.accentHex }}>
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="min-w-0 flex-1">
            ...
            <h3 className="text-white font-extrabold text-[16px] sm:text-[19px] leading-tight truncate" style={{ fontFamily: "'Sora',sans-serif" }}>{hospital.name}</h3>
        ...
        <div className="cw-wrap" style={panelStyle}>
          ...
          <div className="cw-input-wrap" style={{ padding: "8px 10px 10px", display: "flex", gap: "8px", alignItems: "center", flexShrink: 0 }}>
            <div style={{
              ...
              borderRadius: "13px", padding: "9px 12px",
              transition: "all 0.2s",
            ...
            <button onClick={send} disabled={loading || !input.trim()} className="cw-send"
              style={{ width: "40px", height: "40px", borderRadius: "13px", border: "none", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#fff", fontSize: "15px", flexShrink: 0 }}>
              ➤
            </button>
          </div>
  ...
  return (
    <div className="relative" ref={ref}>
      <button onClick={() => setOpen(o => !o)} className="flex items-center gap-2 px-2 py-1.5 rounded-xl hover:bg-slate-50 transition-all border border-slate-200">
        ...
        <svg className={`w-3 h-3 text-slate-400 transition-transform ${open ? "rotate-180" : ""}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
      </button>
      {open && (
  ...
  return (
    <div className="min-h-screen" style={{ background: "#f8faf9", fontFamily: "'DM Sans', sans-serif" }}>
          ...
          <div className="md:hidden mobile-menu-in bg-white border-t border-slate-100 px-4 py-3 flex flex-col gap-2">
            {[["#hospitals", "🏥 Hospitals"], ["#emergency", "🚨 Emergency"]].map(([href, label]) => (
              <a key={href} href={href} onClick={() => setMobileMenuOpen(false)}
                className="py-2.5 px-3 rounded-xl text-[13px] font-semibold text-slate-600 hover:bg-slate-50 transition-all">{label}</a>
            ))}
            {!sessionLoading && !isLoggedIn && (
              <Link href="/sign-in" onClick={() => setMobileMenuOpen(false)}
                className="py-2.5 px-3 rounded-xl text-[13px] font-semibold text-slate-600 hover:bg-slate-50 transition-all">Sign in</Link>
            )}
          </div>
        )}
      </header>
      ...
      <section className="relative bg-white overflow-hidden pt-12 sm:pt-16 pb-16 sm:pb-20">
        ...
        <div className="absolute -right-40 -top-32 w-[500px] sm:w-[700px] h-[500px] sm:h-[600px] rounded-full pointer-events-none" style={{ background: "radial-gradient(ellipse at center,#dcfce7 0%,transparent 65%)", opacity: 0.7 }} />
        <div className="relative max-w-[1400px] mx-auto px-4 sm:px-6 text-center">
          <div className="fade-up inline-flex items-center gap-2 border text-[9px] sm:text-[10px] font-black px-3.5 py-1.5 rounded-full mb-6 uppercase tracking-[0.18em]"
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto">
              <button onClick={requestLocation} disabled={locationStatus === "loading"}
                ...
                {locationStatus === "loading"
                  ...
                  ? <><span className="w-2 h-2 rounded-full bg-emerald-500" />📍 {userAddress ? userAddress.split(",")[0] : "Active"}</>
                  : <>📍 Use my location</>}
              </button>
          ...
          {locationStatus === "denied" && (
            <p className="mb-6 text-[12px] text-rose-500">
              Location denied — <button onClick={() => setShowMapPicker(true)} className="underline font-semibold">pick manually on map</button> instead.
            </p>
          )}
          ...
          <div className="grid grid-cols-3 gap-4 max-w-xs mx-auto">
            {[{ v: "Live", l: "OSM Hospital Data" }, { v: "<2 min", l: "Average match" }, { v: "98%", l: "Satisfaction" }].map(s => (
              <div key={s.l} className="text-center">
                <p className="display text-xl sm:text-2xl font-extrabold text-slate-900">{s.v}</p>
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6">
            ...
            {mapAsSidePanel && selectedHospital && (
              <div className="w-1/2 sticky top-20 rounded-2xl overflow-hidden shadow-[0_8px_40px_rgba(0,0,0,0.1)] slide-right flex flex-col"
                ...
                <MapPanel hospital={selectedHospital} onClose={() => { setMapOpen(false); setSelectedHospital(null); }}
                  userLat={userLocation?.lat} userLng={userLocation?.lng} isMobileSheet={false} />
              </div>
            )}
          </div>
          ...
          {mapAsSheet && selectedHospital && (
            <MapPanel hospital={selectedHospital}
              ...
              userLat={userLocation?.lat} userLng={userLocation?.lng} isMobileSheet={true} />
          )}
        </div>
      </section>
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6">
          <div className="rounded-2xl overflow-hidden grid grid-cols-1 md:grid-cols-2" style={{ background: "linear-gradient(135deg,#f0fdf4,#dcfce7)", border: "1.5px solid #bbf7d0" }}>
            <div className="p-6 sm:p-8 lg:p-10 flex flex-col justify-center">
              ...
              <Link href={isLoggedIn ? "/profile" : "/sign-up"}
      ...
      {/* ── EMERGENCY BANNER ── */}
      <section id="emergency" className="py-12 sm:py-16" style={{ background: "linear-gradient(135deg,#dc2626,#b91c1c)" }}>
        <div className="max-w-3xl mx-auto px-4 sm:px-6 text-center">
          <p className="text-white/50 text-[10px] font-black uppercase tracking-[0.2em] mb-3">Emergency Mode</p>
          <h2 className="display text-white text-[22px] sm:text-2xl lg:text-[38px] font-extrabold mb-4 leading-tight">Life-threatening situation?</h2>
          <p className="text-red-100 text-[13px] sm:text-[15px] mb-7 max-w-md mx-auto leading-relaxed">One tap finds your nearest equipped hospital and alerts them before you arrive.</p>
          <button onClick={() => setEmergencyOpen(true)}
          ...
          <p className="text-white/40 text-[11px] mt-5">Or call <strong>112</strong> immediately for life-threatening emergencies</p>
        </div>
      </section>
